...
- [ ] T003 [P] Create `Makefile` at repository root with targets: `build` (compiles all four binaries to `bin/mq`, `bin/streamer`, `bin/collector`, `bin/api-gateway`), `test` (runs `go test -coverprofile=coverage.out ./...` and prints per-package coverage), `swagger` (runs `swag init -g cmd/api-gateway/main.go -o specs/001-elastic-gpu-telemetry-pipeline/contracts/`), `docker-build` (builds `deploy/docker/*.Dockerfile` images), `lint` (runs `golangci-lint run ./...`), `clean` (removes `bin/` and `coverage.*`)
...
- [ ] T031 [US5] Add `swagger` Makefile target: `swag init -g cmd/api-gateway/main.go -o specs/001-elastic-gpu-telemetry-pipeline/contracts/` and run it to verify it produces `contracts/swagger.json` and `contracts/swagger.yaml`; ensure the `swag` CLI is listed in quickstart.md Prerequisites (already listed — no change needed)
